...
- Etapa concluída: atualidade e cobertura das pesagens de Corte.
- Progresso: 100%.
...
- Checkpoint publicado: `f3fc0e0` — `feat(beef): signal weight data freshness`.
- Próximo marco: retomar a inspeção visual Windows da entrada local-primeiro quando a cadeia MSBuild estiver disponível. O APK atual permanece preparado para uma atualização acumulada do celular.

## Próximo pacote planejado

Retomar a inspeção visual Windows da entrada local-primeiro quando a cadeia MSBuild estiver disponível, sem instalar o APK até uma atualização acumulada do celular.
