...
| Inteligência: decisão vinculada a Anotações e Agenda | Homologação manual final | 99% | Uma decisão gera anotação com referência persistente à recomendação e a anotação aproveita a promoção idempotente já existente para a Agenda. Frontend, rota, modelo, migração e contratos passaram; o Render está no commit `2121e91`, aprovou migrations no startup e a Central carregou em sessão autenticada no Windows. Falta somente salvar uma recomendação real como anotação, ação deixada manual para não criar dado artificial na fazenda. |
| Inteligência: snapshot único por fazenda | Homologação manual final | 99% | A Central guarda um contexto oficial por atualização e envia seu identificador às prioridades e aos cenários. O servidor valida empresa e fazenda antes de reutilizar o snapshot, evitando leituras cruzadas ou dados de momentos diferentes. Contratos Flutter e servidor aprovados; o commit `2121e91` está Live e a Central carregou em sessão autenticada no Windows. Falta conferir visualmente uma prioridade após atualizar a Central. |
