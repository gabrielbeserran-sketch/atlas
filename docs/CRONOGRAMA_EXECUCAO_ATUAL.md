...
| Inteligência: decisão vinculada a Anotações e Agenda | Aguardando deploy de produção | 95% | Uma decisão gera anotação com referência persistente à recomendação e a anotação aproveita a promoção idempotente já existente para a Agenda. Frontend, rota, modelo, migração e contratos passaram; os commits estão no GitHub, mas o Render permanece no commit `04a2ca4`, anterior a esta entrega. Falta publicar e então exercitar o fluxo contra a API. |
| Inteligência: snapshot único por fazenda | Aguardando deploy de produção | 95% | A Central guarda um contexto oficial por atualização e envia seu identificador às prioridades e aos cenários. O servidor valida empresa e fazenda antes de reutilizar o snapshot, evitando leituras cruzadas ou dados de momentos diferentes. Contratos Flutter e servidor aprovados; o backend remoto ainda está no commit `04a2ca4`. |
...
| Produção: auditoria de publicação e disponibilidade | Concluído | 100% | Render está configurado para auto-deploy no branch `master`, mas permaneceu no commit `04a2ca4`; há dois commits de backend pendentes de implantação (`b07a2fb`, `2121e91`). Após ativação, a rota de saúde respondeu 200 em 1,09 s. O primeiro acesso pode exceder 50 s porque o plano Free hiberna a instância. |
