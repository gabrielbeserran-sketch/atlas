...
| Inteligência: decisão vinculada a Anotações e Agenda | Publicado, sessão funcional pendente | 98% | Uma decisão gera anotação com referência persistente à recomendação e a anotação aproveita a promoção idempotente já existente para a Agenda. Frontend, rota, modelo, migração e contratos passaram; o Render está no commit `2121e91`, aprovou migrations no startup e a rota publicada está protegida. Falta apenas exercitar a criação com uma sessão autenticada, sem gerar dados de teste desnecessários. |
| Inteligência: snapshot único por fazenda | Publicado, sessão funcional pendente | 98% | A Central guarda um contexto oficial por atualização e envia seu identificador às prioridades e aos cenários. O servidor valida empresa e fazenda antes de reutilizar o snapshot, evitando leituras cruzadas ou dados de momentos diferentes. Contratos Flutter e servidor aprovados; o commit `2121e91` está Live no Render. |
