...
class AtlasOrchestratorEngine {
  Future<AtlasOrchestratorExecution> execute(
    ...
    List<AtlasOrchestratorTask> tasks = List<AtlasOrchestratorTask>.from(source)
      ..sort(
        (AtlasOrchestratorTask a, AtlasOrchestratorTask b) =>
    ...
    for (int index = 0; index < tasks.length; index++) {
      ...
      tasks[index] = tasks[index].copyWith(
        status: AtlasPipelineStatus.running,
        message: 'Executando...',
      );
    ...
    final int enabled = tasks
        .where((AtlasOrchestratorTask task) => task.enabled)
        .length;
    final int successful = tasks
        .where(
    ...
    final AtlasOrchestratorRun run = AtlasOrchestratorRun(
      ...
      status: successful == enabled
          ? AtlasPipelineStatus.success
          : AtlasPipelineStatus.warning,
