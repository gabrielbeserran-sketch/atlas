...
class AtlasOrchestratorDashboard extends StatefulWidget {
  ...
  @override
  State<AtlasOrchestratorDashboard> createState() => _AtlasOrchestratorDashboardState();
}

class _AtlasOrchestratorDashboardState extends State<AtlasOrchestratorDashboard> {
  ...
  Future<void> _execute() async {
    ...
    final AtlasOrchestratorExecution execution = await _engine.execute(
      _tasks,
          ...
          : RefreshIndicator(
              ...
              child: ListView(
                ...
                children: <Widget>[
                  ...
                  const Text('Pipeline de execução', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                  const SizedBox(height: 10),
                  ...List<Widget>.generate(_tasks.length, _buildTask),
                  const SizedBox(height: 20),
                  const Text('Histórico recente', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                  const SizedBox(height: 10),
                  if (_runs.isEmpty)
                    const Card(child: Padding(padding: EdgeInsets.all(18), child: Text('Nenhuma execução registrada.')))
                  else
  ...
  Widget _buildSummary() {
    final int enabled = _tasks.where((AtlasOrchestratorTask task) => task.enabled).length;
    final int successful = _tasks.where((AtlasOrchestratorTask task) => task.status == AtlasPipelineStatus.success).length;
    final int average = _tasks.isEmpty ? 0 : _tasks.fold<int>(0, (int total, AtlasOrchestratorTask task) => total + task.durationMs) ~/ _tasks.length;
    return Card(
      child: Padding(
        padding: const EdgeInsets.all(18),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[
          const Text('Orquestração central', style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
          const SizedBox(height: 6),
          const Text('Coordena a coleta, análise, decisão, automação e auditoria dos principais motores do Atlas.'),
          const SizedBox(height: 18),
          Wrap(spacing: 12, runSpacing: 12, children: <Widget>[
            _metric('Etapas ativas', '$enabled', Icons.account_tree_outlined),
            _metric('Concluídas', '$successful', Icons.check_circle_outline),
  ...
  Widget _buildTask(int index) {
    ...
    return Card(
      child: ListTile(
        ...
        subtitle: Text('${task.module} • ${_statusLabel(task.status)}${task.durationMs > 0 ? ' • ${task.durationMs} ms' : ''}'),
        trailing: Switch(value: task.enabled, onChanged: _running ? null : (bool value) => _toggleTask(index, value)),
      ),
    );
  }

  Widget _buildRun(AtlasOrchestratorRun run) {
    final String date = '${run.finishedAt.day.toString().padLeft(2, '0')}/${run.finishedAt.month.toString().padLeft(2, '0')} ${run.finishedAt.hour.toString().padLeft(2, '0')}:${run.finishedAt.minute.toString().padLeft(2, '0')}';
    return Card(
      child: ListTile(
        leading: const Icon(Icons.route_outlined),
        title: Text('${run.successfulTasks}/${run.totalTasks} etapas concluídas'),
  ...
  String _statusLabel(AtlasPipelineStatus status) {
    switch (status) {
      case AtlasPipelineStatus.idle: return 'Aguardando';
      case AtlasPipelineStatus.running: return 'Executando';
      case AtlasPipelineStatus.success: return 'Concluído';
      case AtlasPipelineStatus.warning: return 'Atenção';
      case AtlasPipelineStatus.failed: return 'Falha';
    }
  }
}
