...
class AtlasEventAnalyticsService {
  ...
  AtlasEventAnalyticsData build({
    ...
    final last7DaysStart = currentTime.subtract(
      const Duration(days: 7),
    );

    final last30DaysStart = currentTime.subtract(
      const Duration(days: 30),
    );

    final last7DaysEvents = entries.where((item) {
      return !item.occurredAt.isBefore(
        last7DaysStart,
      );
    }).length;
  ...
  List<AtlasEventAnalyticsRankingItem>
      _ranking({
  ...
  List<AtlasEventAnalyticsTypeItem>
      _typeRanking(
  ...
  List<AtlasEventAnalyticsDailyPoint>
      _dailyTrend({
    required List<AtlasEventLogEntry> entries,
  ...
  List<AtlasEventAnalyticsCriticalPattern>
      _criticalPatterns(
    List<AtlasEventLogEntry> entries,
  ) {
    ...
    final candidates = grouped.values
  ...
  List<AtlasEventAnalyticsRecommendation>
      _recommendations({
    required List<AtlasEventLogEntry> entries,
    required Map<AtlasEventPriority, int>
        priorityDistribution,
  }) {
    final result =
    ...
    if (criticalCount > 0) {
      result.add(
        AtlasEventAnalyticsRecommendation(
          ...
          description:
              'Existem $criticalCount eventos críticos registrados.',
          priority:
              AtlasEventPriority.critical,
          reason:
              'Eventos críticos exigem resposta imediata e acompanhamento.',
        ),
      );
    }
    ...
    if (highCount >= 5) {
      result.add(
        AtlasEventAnalyticsRecommendation(
          position: 0,
          title:
              'Reduzir concentração de riscos altos',
          description:
              'Foram identificados $highCount eventos de prioridade alta.',
          priority:
              AtlasEventPriority.high,
          reason:
              'O volume elevado pode indicar gargalos recorrentes.',
        ),
      );
    }
    ...
    final inventoryAlerts =
        entries.where((item) {
      return item.type ==
              AtlasEventType.inventoryLowStock ||
          item.type ==
              AtlasEventType.inventoryOutOfStock;
    }).length;
    ...
    if (inventoryAlerts >= 2) {
      result.add(
        AtlasEventAnalyticsRecommendation(
          ...
          description:
              '$inventoryAlerts alertas de estoque foram registrados.',
          priority:
              AtlasEventPriority.high,
          reason:
              'Alertas recorrentes indicam falha de reposição ou previsão.',
        ),
      );
    }

    final workflowDelays =
        entries.where((item) {
      return item.type ==
              AtlasEventType.taskDelayed ||
          item.type ==
              AtlasEventType.workflowDelayed;
    }).length;
    ...
    if (workflowDelays >= 2) {
      result.add(
        AtlasEventAnalyticsRecommendation(
          position: 0,
          title:
              'Reavaliar capacidade de execução',
          description:
    ...
    final diseaseAlerts =
        entries.where((item) {
      return item.type ==
          AtlasEventType.diseaseAlertCreated;
    }).length;
    ...
    if (diseaseAlerts > 0) {
      result.add(
        AtlasEventAnalyticsRecommendation(
          position: 0,
          title:
              'Priorizar revisão sanitária',
          description:
              '$diseaseAlerts alertas sanitários foram registrados.',
          priority:
              AtlasEventPriority.critical,
          reason:
              'Ocorrências sanitárias podem impactar produção e custos.',
        ),
      );
    }
    ...
    if (result.isEmpty && entries.isNotEmpty) {
      result.add(
        const AtlasEventAnalyticsRecommendation(
          ...
          description:
              'Não foram identificados padrões críticos recorrentes.',
          priority:
