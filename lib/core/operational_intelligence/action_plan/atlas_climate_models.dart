...
class AtlasClimateObservation {
  ...
  factory AtlasClimateObservation.fromMap(
...
class AtlasClimateForecast {
  ...
  factory AtlasClimateForecast.fromMap(
    Map<String, dynamic> map,
  ) {
    double value(String key) =>
        (map[key] as num?)?.toDouble() ?? 0;
    ...
    return AtlasClimateForecast(
      id: map['id']?.toString() ?? '',
      forecastAt: DateTime.tryParse(
            map['forecastAt']?.toString() ?? '',
          ) ??
          DateTime.now(),
      ...
      probabilityOfRainPercent:
          value('probabilityOfRainPercent'),
