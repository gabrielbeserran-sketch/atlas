...
class AtlasStrategyPerformanceService {
  ...
  List<AtlasAreaPerformance> buildAreas({
    ...
    return AtlasOperationalArea.values
        .map((area) {
          ...
          final areaGoals = goals.where((goal) => goal.area == area).toList();

          final completed = areaActions
              .where((action) => action.isCompleted)
              .length;
          final open = areaActions.where((action) => action.isOpen).length;
          ...
          final activeGoals = areaGoals.where((goal) => goal.active).length;
          final completedGoals = areaGoals
  ...
  List<AtlasGoalPerformanceSummary> buildGoals({
    ...
    return goals
        .map((goal) {
          final goalLinks = links.where((link) => link.goalId == goal.id);
  ...
  AtlasFarmExecutionScore buildFarmScore({
    ...
    final completion = total == 0 ? 0.0 : completed / total * 100;
    final progress = total == 0
        ...
        : actions
                  ...
                  .fold<int>(0, (a, b) => a + b) /
              total;
    final deadline = total == 0 ? 100.0 : (1 - overdue / total) * 100;
    final responsibility = total == 0 ? 0.0 : withResponsible / total * 100;
