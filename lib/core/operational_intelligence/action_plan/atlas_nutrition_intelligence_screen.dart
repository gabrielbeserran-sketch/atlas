  ...
  Widget build(BuildContext context) {
    ...
    final avgGain = records.isEmpty
        ...
        : records.map((e) => e.averageDailyGainKg).reduce((a, b) => a + b) /
            records.length;
    final validConversions = records.where((e) => e.feedConversion > 0).toList();
    final avgConversion = validConversions.isEmpty
        ? 0.0
        : validConversions.map((e) => e.feedConversion).reduce((a, b) => a + b) /
            ...
            : TabBarView(
                children: [
                  ...
                  _metrics([
                    ...
                    ('Dietas ativas', diets.where((e) => e.active).length.toDouble(), ''),
                  ]),
                  _list(
                    records.map((e) => ListTile(
                          title: Text(e.lotName),
                          subtitle: Text(
                            '${DateFormat('dd/MM/yyyy').format(e.recordedAt)} • '
                            '${e.consumptionPerAnimalKg.toStringAsFixed(2)} kg/cab',
                          ),
                          trailing: Text('CA ${e.feedConversion.toStringAsFixed(2)}'),
                        )),
                  ...
                  _metrics([
                    ...
                    ('Eficiência', avgConversion <= 0 ? 0 : 100 / avgConversion, '%'),
                  ]),
                  _list(
                    [
                  ...
                  _list(
                    [
                      const ListTile(
                        ...
                        subtitle: Text('Negociar ingredientes de maior participação.'),
                      ),
                      const ListTile(
                        leading: Icon(Icons.science_outlined),
                      ...
                      const ListTile(
                        ...
                        subtitle: Text('Ajustar a oferta diária ao consumo real.'),
                      ),
                    ],
                    '',
                  ),
