...
class AtlasMeetingDecisionMonitoringController extends ChangeNotifier {
  AtlasMeetingDecisionMonitoringController({
    ...
    this.farmName,
           ...
           meetingService ?? AtlasExecutionMeetingService.instance,
       _monitoringService = monitoringService;

  final AtlasExecutionMeetingService _meetingService;
  final AtlasMeetingDecisionMonitoringService _monitoringService;
  ...
  List<AtlasMeetingDecisionMonitoringItem> get items =>
      List<AtlasMeetingDecisionMonitoringItem>.unmodifiable(_items);
  ...
  int get overdueCount => _items.where((item) => item.isOverdue).length;

  int get dueSoonCount => _items.where((item) => item.isDueSoon).length;

  int get withoutResponsibleCount =>
      _items.where((item) => !item.hasResponsible).length;

  int get withoutLinkedActionCount =>
      _items.where((item) => !item.hasLinkedAction).length;
  ...
  Future<void> load() async {
    ...
    try {
      ...
      _items = _monitoringService.build(meetings: meetings);
    } catch (error) {
