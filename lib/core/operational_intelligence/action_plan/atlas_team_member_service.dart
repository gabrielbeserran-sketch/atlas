...
class AtlasTeamMemberService {
  ...
  static final AtlasTeamMemberService instance = AtlasTeamMemberService._();

  static const String _storageKey = 'atlas_team_members_v1';
  ...
  Future<List<AtlasTeamMember>> load({
    ...
    final filtered = all.where((member) {
      final matchesFarm =
          ...
          member.farmName?.trim().toLowerCase() == normalizedFarm;
      final matchesActive = includeInactive || member.active;
  ...
  Future<AtlasTeamMember> save(AtlasTeamMember member) async {
    final all = await _loadAll();
    final index = all.indexWhere((item) => item.id == member.id);
  ...
  Future<void> setActive({
    ...
    await save(member.copyWith(active: active));
  }
  ...
  Future<List<AtlasTeamMember>> _loadAll() async {
    ...
    try {
      ...
      return decoded
          .map(
            (item) =>
                AtlasTeamMember.fromMap(Map<String, dynamic>.from(item as Map)),
          )
          .toList();
    } catch (_) {
      return <AtlasTeamMember>[];
  ...
  Future<void> _saveAll(List<AtlasTeamMember> members) async {
    await _preferences.setString(
      _storageKey,
      jsonEncode(members.map((member) => member.toMap()).toList()),
    );
  }
}
