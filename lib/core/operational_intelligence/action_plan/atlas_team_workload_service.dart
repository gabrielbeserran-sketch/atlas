    ...
    final workloads =
        members.map((member) {
          final memberActions = actions.where((action) {
            ...
            return action.responsibleName.trim().toLowerCase() ==
                member.name.trim().toLowerCase();
          }).toList();

          final open = memberActions.where((action) => action.isOpen).length;
          final overdue = memberActions
              .where((action) => action.isOverdue)
              .length;
          final critical = memberActions
              .where(
                (action) => action.priority == AtlasCanonicalPriority.critical,
              )
              .length;
          ...
          return AtlasTeamWorkload(
            member: member,
            actions: List<AtlasCommandCenterAction>.unmodifiable(memberActions),
            openActions: open,
          ...
          (first, second) =>
              second.workloadScore.compareTo(first.workloadScore),
        );

    return List<AtlasTeamWorkload>.unmodifiable(workloads);
  }
}
