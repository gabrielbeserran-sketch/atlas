...
class AtlasActionAttentionController
  ...
  int get criticalCount => _items
      .where(
        (item) =>
            item.severity ==
            AtlasActionAttentionSeverity.critical,
      )
      .length;
  ...
  int get warningCount => _items
      .where(
        (item) =>
            item.severity ==
            AtlasActionAttentionSeverity.warning,
      )
      .length;
  ...
  Future<void> rebuild({
    ...
    final snoozed =
        await _preferences.loadSnoozedUntil();
    final now = DateTime.now();

    _items = _service
        .build(
          actions: actions,
          latestUpdateDates: latestUpdateDates,
        )
        .where((attention) {
