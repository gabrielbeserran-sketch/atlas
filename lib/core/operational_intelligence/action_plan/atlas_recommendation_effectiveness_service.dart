...
class AtlasRecommendationEffectivenessService {
  ...
  List<AtlasRecommendationEffectiveness> build({
    ...
    final modules = actions
        .map((action) => action.sourceModule)
        .toSet();

    final ranking = modules.map((module) {
      final moduleActions = actions
          .where(
            (action) => action.sourceModule == module,
          )
          .toList();
      ...
      final evidenceCoverage = moduleActions.isEmpty
          ...
          : moduleOutcomes.length /
              ...
              100;
      final normalizedRoi =
          averageRoi.clamp(-100.0, 200.0);
      final roiScore =
          ((normalizedRoi + 100) / 3).clamp(0.0, 100.0);
      ...
      ..sort(
        (first, second) =>
            second.effectivenessScore.compareTo(
          first.effectivenessScore,
        ),
      );

    return List<AtlasRecommendationEffectiveness>.unmodifiable(
      ranking,
    );
  }
}
