...
class AtlasSprints2125Service {
  AtlasSprints2125Service({AtlasHttpClient? client}) : _client = client ?? AtlasHttpClient();
  final AtlasHttpClient _client;

  Future<AtlasSprints2125DashboardData> loadDashboard(String farmId) async {
    final id = farmId.trim();
    if (id.isEmpty) throw ArgumentError.value(farmId, 'farmId', 'Informe uma fazenda válida.');
    ...
    return AtlasSprints2125DashboardData.fromJson({
      'precision': responses[0].asMap(), 'reproduction': responses[1].asMap(),
      'health': responses[2].asMap(), 'nutrition': responses[3].asMap(),
