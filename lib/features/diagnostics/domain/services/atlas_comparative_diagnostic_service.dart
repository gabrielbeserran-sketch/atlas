...
class AtlasComparativeDiagnosticService {
  ...
  AtlasComparativeDiagnosticData buildComparison({
    ...
    final validDiagnostics = diagnostics
        .where((item) {
    ...
    final areaComparisons =
        _buildAreaComparisons(
      diagnostics: validDiagnostics,
    );
  ...
  List<AtlasComparativeFarmRanking>
      _buildRanking({
    required List<AtlasDiagnosticData>
        diagnostics,
    ...
    return List.generate(
      ...
      (index) {
        ...
        final criticalRiskCount =
            item.risks.where((risk) {
          return risk.level ==
              AtlasDiagnosticLevel.critical;
        }).length;
  ...
  List<AtlasComparativeHighlight>
      _buildHighlights({
    required List<AtlasDiagnosticData>
        diagnostics,
    required List<AtlasComparativeFarmRanking>
        ranking,
    required List<AtlasComparativeAreaData>
        areas,
    required double averageScore,
  }) {
    final highlights =
    ...
    if (ranking.isNotEmpty) {
      ...
      highlights.add(
        AtlasComparativeHighlight(
          id: 'leader_farm',
          title:
              '${leader.farmName} lidera a operação',
          description:
          ...
          type:
              AtlasComparativeHighlightType.leader,
          level:
      ...
      if (critical.score < averageScore) {
        highlights.add(
          AtlasComparativeHighlight(
            id: 'critical_farm',
            title:
                '${critical.farmName} exige maior atenção',
            description:
                'A propriedade está '
            ...
            type:
                AtlasComparativeHighlightType.warning,
    ...
    for (final area in areas) {
      if (area.amplitude >= 20) {
        highlights.add(
          AtlasComparativeHighlight(
            id:
                'imbalance_${area.area.name}',
            title:
                'Grande diferença em ${area.title.toLowerCase()}',
            description:
            ...
            type:
                AtlasComparativeHighlightType.imbalance,
            level:
    ...
    final opportunityDiagnostic =
        [...diagnostics]
          ..sort(
            (first, second) =>
                second.opportunities.length
                    .compareTo(
    ...
    if (opportunityDiagnostic.isNotEmpty &&
        opportunityDiagnostic
            .first.opportunities.isNotEmpty) {
      final farm =
          opportunityDiagnostic.first;

      highlights.add(
        AtlasComparativeHighlight(
          id: 'opportunity_farm',
          title:
              '${farm.scopeLabel} concentra oportunidades',
          description:
          ...
          type:
              AtlasComparativeHighlightType.opportunity,
          level:
  ...
  List<AtlasComparativePriority>
    ...
    for (final diagnostic in diagnostics) {
      final scoreGap =
          (averageScore - diagnostic.score)
              .clamp(0.0, 100.0);

      final criticalRiskCount =
          diagnostic.risks.where((risk) {
        return risk.level ==
            AtlasDiagnosticLevel.critical;
      }).length;

      final priorityScore =
          diagnostic.mainPriority.score *
                  0.55 +
              scoreGap * 0.25 +
              criticalRiskCount * 6 +
              diagnostic.bottlenecks.length *
                  3;
      ...
      candidates.add(
        _PriorityCandidate(
          farmName:
              diagnostic.scopeLabel,
          priority:
              diagnostic.mainPriority,
          score: priorityScore.clamp(
            0.0,
            100.0,
          ),
        ),
