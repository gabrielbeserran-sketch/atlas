...
extension AtlasSupplyLogisticsModuleX
    on AtlasSupplyLogisticsModule {
...
class AtlasSupplyLogisticsRecord {
  ...
  factory AtlasSupplyLogisticsRecord.fromMap(
    Map<String, dynamic> map,
  ) {
    final code = map['module']?.toString() ?? '';
    final module =
        AtlasSupplyLogisticsModule.values.firstWhere(
      (item) => item.code == code,
      orElse: () =>
          AtlasSupplyLogisticsModule.intelligentPurchasing,
    );
    ...
    return AtlasSupplyLogisticsRecord(
      ...
      unitCost:
          (map['unitCost'] as num?)?.toDouble() ?? 0,
      freightCost:
          (map['freightCost'] as num?)?.toDouble() ?? 0,
      plannedValue:
          (map['plannedValue'] as num?)?.toDouble() ?? 0,
      actualValue:
          (map['actualValue'] as num?)?.toDouble() ?? 0,
      progressPercent:
