...
class AtlasDecisionTrackingScreen extends StatefulWidget {
  ...
  final ValueChanged<List<AtlasDecisionExecution>>? onExecutionsChanged;
...
class _AtlasDecisionTrackingScreenState
  ...
  AtlasDecisionTrackingData get summary {
    return service.buildSummary(executions: executions);
  }

  List<String> get farms {
    final result = executions.map((item) => item.farmName).toSet().toList()
      ..sort();

    return result;
  }

  List<AtlasDecisionExecution> get filteredExecutions {
  ...
  void _updateExecution(AtlasDecisionExecution updated) {
    setState(() {
    ...
    widget.onExecutionsChanged?.call(executions);
  }

  Future<void> _registerResult(AtlasDecisionExecution execution) async {
    final controller = TextEditingController(
      text: execution.realizedFinancialImpact.toStringAsFixed(2),
    );
    ...
    final result = await showDialog<_ExecutionResultInput>(
      context: context,
            ...
            child: data.hasData
                ? ListView(
                    ...
                    children: [
                      ...
                      const _SectionTitle(
                        title: 'Decisões em acompanhamento',
                        subtitle:
                        ...
                        ...filteredExecutions.map((execution) {
                          return Padding(
                            ...
                            child: _ExecutionCard(
                              ...
                              onStart: () {
                                _updateExecution(
                                  service.start(execution: execution),
                                );
                              },
                              onToggleStep: (position) {
                                _updateExecution(
                                  service.toggleStep(
                                    execution: execution,
                                    position: position,
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: const LinearGradient(
          colors: [Color(0xFF102A43), Color(0xFF243B53), Color(0xFF486581)],
        ),
            ...
            children: [
              ...
              Wrap(
                ...
                children: [
                  _HeroMetric(
                    label: 'Ativas',
                    value: data.activeExecutions.length,
                  ),
                  _HeroMetric(
                    label: 'Concluídas',
                    value: data.completedExecutions.length,
                  ),
                ],
              ),
            ],
...
class _TrackingFilters extends StatelessWidget {
  ...
  final AtlasDecisionExecutionStatus? selectedStatus;

  final ValueChanged<String?> onFarmChanged;

  final ValueChanged<AtlasDecisionExecutionStatus?> onStatusChanged;
      ...
      child: Padding(
        ...
        child: Wrap(
          ...
          children: [
            ...
            SizedBox(
              width: 230,
              child: DropdownButtonFormField<AtlasDecisionExecutionStatus?>(
                ...
                items: [
                  ...
                  ...AtlasDecisionExecutionStatus.values.map((status) {
                    return DropdownMenuItem(
                      value: status,
                      child: Text(atlasDecisionExecutionStatusLabel(status)),
                    );
                  }),
                ],
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Icon(Icons.task_alt_outlined, color: color, size: 29),
                ...
                Text(
                  atlasDecisionExecutionStatusLabel(execution.status),
            ...
            Wrap(
              ...
              runSpacing: 8,
              children: [
                if (execution.status == AtlasDecisionExecutionStatus.approved)
                  FilledButton.icon(
                    onPressed: onStart,
                ...
                if (execution.status !=
                    ...
                    execution.status != AtlasDecisionExecutionStatus.completed)
                  OutlinedButton.icon(
...
class _ExecutionResultInput {
  const _ExecutionResultInput({required this.value, required this.summary});

  final double value;
  final String summary;
}

Color _statusColor(AtlasDecisionExecutionStatus status) {
  switch (status) {
