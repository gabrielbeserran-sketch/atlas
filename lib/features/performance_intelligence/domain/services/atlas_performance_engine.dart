...
class AtlasPerformanceEngine {
  ...
  AtlasPerformanceScorecard analyze(List<AtlasPerformanceKpi> kpis) {
    ...
    double categoryScore(AtlasKpiCategory category) {
      final values = evaluations.where((e) => e.kpi.category == category).map((e) => e.achievement).toList();
    ...
    for (final evaluation in evaluations) {
      ...
      if (evaluation.status == AtlasKpiStatus.critical) {
        alerts.add(AtlasPerformanceAlert(
          ...
          message: 'Resultado atual de ${_format(kpi.currentValue)} ${kpi.unit}, frente à meta de ${_format(kpi.targetValue)} ${kpi.unit}.',
        ...
        alerts.add(AtlasPerformanceAlert(
          ...
          message: 'O indicador atingiu ${evaluation.achievement.toStringAsFixed(0)}% do desempenho esperado.',
          severity: AtlasPerformanceAlertSeverity.attention,
          kpiId: kpi.id,
        ));
      }
      final worsening = kpi.direction == AtlasKpiDirection.lowerIsBetter ? kpi.variation > 8 : kpi.variation < -8;
      if (worsening) {
        alerts.add(AtlasPerformanceAlert(
          title: 'Tendência negativa em ${kpi.name}',
          message: 'Variação de ${kpi.variation.toStringAsFixed(1)}% em relação ao período anterior.',
          severity: AtlasPerformanceAlertSeverity.attention,
          kpiId: kpi.id,
        ));
      }
    }
    final overall = evaluations.isEmpty ? 0.0 : evaluations.map((e) => e.achievement).reduce((a, b) => a + b) / evaluations.length;
  ...
  AtlasKpiEvaluation _evaluate(AtlasPerformanceKpi kpi) {
    ...
    switch (kpi.direction) {
      case AtlasKpiDirection.higherIsBetter:
        achievement = kpi.targetValue <= 0 ? 0 : (kpi.currentValue / kpi.targetValue) * 100;
      case AtlasKpiDirection.lowerIsBetter:
        achievement = kpi.currentValue <= 0 ? 120 : (kpi.targetValue / kpi.currentValue) * 100;
      case AtlasKpiDirection.targetRange:
        ...
        if (kpi.currentValue >= min && kpi.currentValue <= max) {
          ...
          final distance = kpi.currentValue < min ? min - kpi.currentValue : kpi.currentValue - max;
          achievement = (100 - (distance / (kpi.targetValue.abs() == 0 ? 1 : kpi.targetValue.abs())) * 100);
        }
    }
    achievement = achievement.clamp(0, 120).toDouble();
    final status = achievement >= 105 ? AtlasKpiStatus.excellent : achievement >= 95 ? AtlasKpiStatus.onTarget : achievement >= 80 ? AtlasKpiStatus.attention : AtlasKpiStatus.critical;
    return AtlasKpiEvaluation(kpi: kpi, status: status, achievement: achievement);
  }
