...
class AtlasPerformanceDashboardScreen extends StatefulWidget {
  ...
  @override
  State<AtlasPerformanceDashboardScreen> createState() =>
      _AtlasPerformanceDashboardScreenState();
}

class _AtlasPerformanceDashboardScreenState
    extends State<AtlasPerformanceDashboardScreen> {
  ...
  Widget build(BuildContext context) {
    final visible = _filter == null
        ? _kpis
        : _kpis.where((e) => e.category == _filter).toList();
          ...
          IconButton(
            onPressed: () => Navigator.of(context).push(
              MaterialPageRoute<void>(
                builder: (_) => AtlasPredictiveAiScreen(farmId: widget.farmId),
              ),
            ),
            tooltip: 'Abrir análise preditiva',
                ...
                child: ListView(
                  ...
                  children: [
                    ...
                    if (visible.isEmpty)
                      ...
                      ...visible.map((kpi) {
                        final evaluation = analysis.evaluations.firstWhere(
                          (e) => e.kpi.id == kpi.id,
                        );
                        return Padding(
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                SizedBox(height: 7),
                Text(
                  'Consolide indicadores produtivos, financeiros, operacionais e estratégicos. Compare meta, resultado, tendência e nível de atenção.',
  ...
  Widget build(BuildContext context) => LayoutBuilder(
    builder: (context, constraints) {
      ...
      final items = <({String label, double value, IconData icon})>[
        ...
        (
          label: 'Produtivo',
          value: scorecard.productiveScore,
          icon: Icons.agriculture_outlined,
        ),
        (
          label: 'Financeiro',
          value: scorecard.financialScore,
          icon: Icons.account_balance_wallet_outlined,
        ),
        (
          label: 'Operacional',
          value: scorecard.operationalScore,
          icon: Icons.precision_manufacturing_outlined,
        ),
        (
          label: 'Estratégico',
          value: scorecard.strategicScore,
...
class _Alerts extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (alerts.isEmpty) {
      return const Card(
        child: ListTile(
          leading: Icon(Icons.verified_outlined, color: Color(0xFF17735F)),
          title: Text('Nenhum alerta relevante'),
          subtitle: Text('Os indicadores estão dentro da faixa esperada.'),
        ),
      );
    }
    return Card(
        ...
        child: Column(
          ...
          children: [
            ...
            ...alerts.take(6).map((alert) {
              final critical =
                  alert.severity == AtlasPerformanceAlertSeverity.critical;
              return ListTile(
...
class _KpiCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final trendPositive = kpi.direction == AtlasKpiDirection.lowerIsBetter
        ? kpi.variation <= 0
        : kpi.variation >= 0;
    return Card(
      child: Padding(
        ...
        child: Column(
          children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        '${_categoryLabel(kpi.category)} • Atualizado em ${DateFormat('dd/MM/yyyy').format(kpi.updatedAt)}',
              ...
              children: [
                Expanded(
                  child: _Metric(
                    ...
                    value: '${_number(kpi.currentValue)} ${kpi.unit}',
                  ),
                ),
                Expanded(
                  child: _Metric(
                    label: 'Meta',
                    value: '${_number(kpi.targetValue)} ${kpi.unit}',
                  ),
                ),
                Expanded(
                  child: _Metric(
                    label: 'Atingimento',
                    value: '${evaluation.achievement.toStringAsFixed(0)}%',
                  ),
                ),
                Expanded(
                  child: _Metric(
                    label: 'Tendência',
                    value:
                        '${kpi.variation >= 0 ? '+' : ''}${kpi.variation.toStringAsFixed(1)}%',
                    icon: trendPositive
                        ? Icons.trending_up
                        : Icons.trending_down,
                    valueColor: trendPositive
                        ? const Color(0xFF17735F)
                        : const Color(0xFFB42318),
                  ),
                ),
            ...
            LinearProgressIndicator(
              value: (evaluation.achievement / 100).clamp(0, 1),
              minHeight: 8,
              borderRadius: BorderRadius.circular(8),
              color: color,
            ),
...
class _EmptyState extends StatelessWidget {
  ...
  Widget build(BuildContext context) => const Card(
    child: Padding(
      ...
      child: Center(child: Text('Nenhum indicador nesta categoria.')),
    ),
  );
}
...
class _KpiDialogState extends State<_KpiDialog> {
  ...
  void initState() {
    ...
    _previous = TextEditingController(
      text: kpi?.previousValue.toString() ?? '0',
    );
  ...
  void _save() {
    ...
    Navigator.pop(
      ...
      AtlasPerformanceKpi(
        ...
        previousValue: _parse(_previous.text),
        ...
        child: SingleChildScrollView(
          child: Column(
            children: [
              TextFormField(
                controller: _name,
                decoration: const InputDecoration(labelText: 'Nome do KPI'),
                validator: (v) =>
                    v == null || v.trim().isEmpty ? 'Informe o nome' : null,
              ),
              const SizedBox(height: 12),
              DropdownButtonFormField<AtlasKpiCategory>(
                initialValue: _category,
                decoration: const InputDecoration(labelText: 'Categoria'),
                items: AtlasKpiCategory.values
                    .map(
                      (e) => DropdownMenuItem(
                        value: e,
              ...
              const SizedBox(height: 12),
              DropdownButtonFormField<AtlasKpiDirection>(
                initialValue: _direction,
                decoration: const InputDecoration(
                  labelText: 'Regra de desempenho',
                ),
                items: AtlasKpiDirection.values
                    .map(
                      (e) => DropdownMenuItem(
                        value: e,
                        child: Text(_directionLabel(e)),
                      ),
                    )
                    .toList(),
