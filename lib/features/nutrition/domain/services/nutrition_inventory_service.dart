...
class NutritionInventoryResult {
  const NutritionInventoryResult({
    required this.plan,
...
class NutritionInventoryService {
  NutritionInventoryService({FarmInventoryStorageService? storage})
    : storage = storage ?? FarmInventoryStorageService();

  final FarmInventoryStorageService storage;

  Future<NutritionInventoryResult> deductDailyConsumption(
    NutritionPlanData plan, {
    required String farmId,
  }) async {
    if (!plan.stockIntegrationEnabled ||
        plan.inventoryDeducted ||
        plan.ingredients.isEmpty) {
      return NutritionInventoryResult(
        plan: plan,
        message: 'Dieta salva sem nova movimentação de estoque.',
        success: true,
      );
    }
    ...
    if (id.isEmpty) {
      return NutritionInventoryResult(
        plan: plan,
        message:
            'Dieta salva, mas a fazenda não está disponível para baixar o estoque.',
        success: false,
      );
    }

    final items = await storage.loadItems(plan.farmName, farmId: id);
    ...
    if (missing.isNotEmpty) {
      return NutritionInventoryResult(
        plan: plan,
        message:
            'Dieta salva, mas estes ingredientes não foram encontrados no estoque: ${missing.join(', ')}.',
        success: false,
      );
    }
    ...
    for (final entry in matches.entries) {
      if (items[entry.key].quantity < entry.value) {
        return NutritionInventoryResult(
          plan: plan,
          message:
              'Dieta salva, mas não há saldo suficiente de ${items[entry.key].name}.',
          success: false,
        );
      }
    }
    ...
    for (final entry in matches.entries) {
      ...
      final movement = FarmInventoryMovement(
        id: 'nutrition_${plan.id}_${item.id}',
        type: 'Saída',
        quantity: entry.value,
        date: plan.startDate,
        responsible: 'Atlas Nutrição',
        reason:
            'Consumo nutricional diário — ${plan.dietName} / ${plan.groupName}',
        document: 'NUT-${plan.id}',
    ...
    final updated = plan.copyWith(
      ...
      inventoryDeductionCost: totalCost,
    );
    return NutritionInventoryResult(
      plan: updated,
      message:
          'Consumo diário baixado do estoque oficial. Custo: R\$ ${totalCost.toStringAsFixed(2).replaceAll('.', ',')}.',
      success: true,
    );
  }
}
