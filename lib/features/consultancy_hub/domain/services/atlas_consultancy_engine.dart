...
class AtlasConsultancySummary {
  const AtlasConsultancySummary({
    required this.total,
    required this.active,
    required this.attention,
    required this.visitsNext30Days,
...
class AtlasConsultancyEngine {
  ...
  AtlasConsultancySummary summarize(List<AtlasConsultancyRecord> items) {
    ...
    final score = items.isEmpty
        ? 0.0
        : items.fold<double>(0, (s, e) => s + e.executiveScore) / items.length;
    return AtlasConsultancySummary(
      total: items.length,
      active: items.where((e) => e.status == AtlasClientStatus.active).length,
      attention: items
          .where((e) => e.status == AtlasClientStatus.attention)
          .length,
      visitsNext30Days: items
          .where(
            (e) => !e.nextVisit.isBefore(now) && !e.nextVisit.isAfter(limit),
          )
          .length,
      averageScore: score,
      monthlyRevenue: items
          .where((e) => e.status != AtlasClientStatus.inactive)
          .fold<double>(0, (s, e) => s + e.monthlyFee),
