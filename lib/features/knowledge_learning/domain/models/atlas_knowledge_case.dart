...
class AtlasKnowledgeCase {
  ...
  AtlasKnowledgeCase copyWith({AtlasKnowledgeStatus? status}) => AtlasKnowledgeCase(
        id: id,
        farmId: farmId,
        farmName: farmName,
        createdAt: createdAt,
  ...
  factory AtlasKnowledgeCase.fromJson(Map<String, dynamic> json) {
    return AtlasKnowledgeCase(
      ...
      lessons: (json['lessons'] as List? ?? const <dynamic>[]).whereType<String>().toList(),
      predictedValue: (json['predictedValue'] as num?)?.toDouble(),
      predictedEconomicGain: (json['predictedEconomicGain'] as num?)?.toDouble(),
      ...
      recommendationImplemented: json['recommendationImplemented'] as bool? ?? true,
...
class AtlasKnowledgeOverview {
  ...
  int get bestPractices => cases.where((item) => item.status == AtlasKnowledgeStatus.bestPractice).length;
  int get implementedRecommendations => cases.where((item) => item.recommendationImplemented).length;

  double get successRate => cases.isEmpty ? 0 : cases.where((item) => item.success).length / cases.length * 100;
  double get implementationRate => cases.isEmpty ? 0 : implementedRecommendations / cases.length * 100;
  double get averageResponseDays => cases.isEmpty ? 0 : cases.fold<double>(0, (sum, item) => sum + item.responseDays) / cases.length;
  double get averageEconomicGain => cases.isEmpty ? 0 : cases.fold<double>(0, (sum, item) => sum + item.economicGain) / cases.length;

  double get averagePredictionAccuracy {
    final values = cases.map((item) => item.predictionAccuracy).whereType<double>().toList();
