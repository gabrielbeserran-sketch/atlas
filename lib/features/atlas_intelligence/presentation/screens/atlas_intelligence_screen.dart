...
class AtlasIntelligenceScreen extends StatelessWidget {
            ...
            child: data.hasData
                ? ListView(
                    ...
                    children: [
                      ...
                      if (data.primaryRecommendation != null) ...[
                        const SizedBox(height: 24),
                        const _SectionTitle(
                          title: 'Recomendação central',
                          subtitle:
                              'A principal orientação gerada pelo cérebro do Atlas.',
                        ),
                        const SizedBox(height: 12),
                        _PrimaryRecommendationCard(
                          item: data.primaryRecommendation!,
                      ...
                      const SizedBox(height: 12),
                      _PatternList(items: data.patterns),
                      const SizedBox(height: 24),
                      const _SectionTitle(
                        title: 'Hipóteses',
                        subtitle:
                      ...
                      const SizedBox(height: 12),
                      _HypothesisList(items: data.hypotheses),
                      const SizedBox(height: 24),
                      const _SectionTitle(
                      ...
                      const SizedBox(height: 24),
                      const _SectionTitle(
                        title: 'Sinais consolidados',
                        subtitle:
                            'Informações relevantes recebidas do Atlas OS.',
                      ),
                      const SizedBox(height: 12),
                      _SignalList(items: data.signals, onOpenFarm: onOpenFarm),
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: const LinearGradient(
          colors: [Color(0xFF030712), Color(0xFF111D33), Color(0xFF1F3B55)],
        ),
          ...
          final information = Column(
            ...
            children: [
              ...
              Wrap(
                ...
                runSpacing: 9,
                children: [
                  _HeroMetric(label: 'Sinais', value: data.signals.length),
                  _HeroMetric(label: 'Padrões', value: data.patterns.length),
                  _HeroMetric(
                    label: 'Hipóteses',
          ...
          final side = Container(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  atlasIntelligenceStatusLabel(data.status),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Icon(Icons.auto_awesome_outlined, color: color, size: 30),
                ...
                Text(
                  atlasIntelligencePriorityLabel(item.priority),
...
class _PatternList extends StatelessWidget {
  const _PatternList({required this.items});
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: items.map((item) {
        return Card(
          child: ListTile(
            leading: const Icon(Icons.hub_outlined, color: Color(0xFF6A1B9A)),
            title: Text(
              item.title,
            ...
            isThreeLine: true,
            trailing: Text(
              item.strengthScore.toStringAsFixed(0),
              style: const TextStyle(
...
class _HypothesisList extends StatelessWidget {
  const _HypothesisList({required this.items});

  final List<AtlasIntelligenceHypothesis> items;
        ...
        return Card(
          child: ExpansionTile(
            ...
            children: [
              ...
              const SizedBox(height: 10),
              ...item.validationSteps.map(
                (step) => Padding(
                  padding: const EdgeInsets.only(bottom: 5),
                  child: Row(
                    children: [
                      const Icon(Icons.check_outlined, size: 17),
...
class _RecommendationList extends StatelessWidget {
  ...
  final List<AtlasIntelligenceRecommendation> items;
...
class _SignalList extends StatelessWidget {
  const _SignalList({required this.items, required this.onOpenFarm});
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: items.map((item) {
        ...
        return Card(
          child: ListTile(
            leading: Icon(Icons.sensors_outlined, color: color),
            title: Text(
              item.title,
              style: const TextStyle(fontWeight: FontWeight.bold),
            ),
...
Color _statusColor(AtlasIntelligenceStatus status) {
  switch (status) {
...
Color _priorityColor(AtlasIntelligencePriority priority) {
  switch (priority) {
...
Color _severityColor(AtlasIntelligenceSeverity severity) {
  switch (severity) {
