...
class AtlasIntelligenceService {
  ...
  AtlasIntelligenceData build({
    ...
    final hypotheses = _buildHypotheses(
      signals: signals,
      patterns: patterns,
    );
  ...
  List<AtlasIntelligenceSignal> _buildSignals(
    AtlasOsData atlasOs,
  ) {
    final result =
    ...
    for (final item in atlasOs.commands) {
      result.add(
        AtlasIntelligenceSignal(
          ...
          source: item.source,
          type:
              AtlasIntelligenceSignalType.operational,
          severity:
              _severityFromPriority(
            item.priority,
          ),
          relevanceScore:
    ...
    for (final item in atlasOs.criticalItems) {
      result.add(
        AtlasIntelligenceSignal(
          ...
          source: 'Atlas OS',
          type:
              AtlasIntelligenceSignalType.predictive,
          severity:
              _severityFromOs(
            item.severity,
          ),
  ...
  List<AtlasIntelligencePattern>
    ...
    final groupedByFarm =
        <String, List<AtlasIntelligenceSignal>>{};
    ...
    for (final entry in groupedByFarm.entries) {
      final highSignals =
        ...
        return item.severity ==
                AtlasIntelligenceSeverity.high ||
            item.severity ==
                AtlasIntelligenceSeverity.critical;
      }).toList();
      ...
      if (highSignals.length >= 2) {
        result.add(
          AtlasIntelligencePattern(
            id:
                'cascade_${entry.key}_${highSignals.length}',
            title:
                'Concentração de riscos em ${entry.key}',
            description:
                '${highSignals.length} sinais de alta severidade estão relacionados à mesma operação.',
            type:
                AtlasIntelligencePatternType.cascading,
            strengthScore:
                math.min(
                  100,
                  55 +
                      highSignals.length * 10,
                ).toDouble(),
    ...
    final executionSignals =
        signals.where((item) {
      return item.type ==
          AtlasIntelligenceSignalType.execution;
    }).toList();

    final operationalSignals =
        signals.where((item) {
      return item.type ==
          AtlasIntelligenceSignalType.operational;
    }).toList();
    ...
    if (positiveImpact > 50000) {
      result.add(
        AtlasIntelligencePattern(
          ...
          title:
              'Oportunidade financeira concentrada',
          description:
              'O conjunto de sinais aponta potencial financeiro relevante nas ações atuais.',
          type:
              AtlasIntelligencePatternType.opportunity,
          strengthScore:
              math.min(
                100,
                positiveImpact / 1000,
    ...
    if (atlasOs.executionPercent < 60 &&
        atlasOs.goalProbabilityPercent >= 75) {
      result.add(
        AtlasIntelligencePattern(
          id: 'execution_goal_contradiction',
          title:
              'Contradição entre metas e execução',
          description:
              'A probabilidade de atingir metas está alta, mas a execução prevista permanece baixa.',
          type:
              AtlasIntelligencePatternType.contradiction,
          strengthScore: 79,
    ...
    result.sort(
      (first, second) =>
          second.strengthScore.compareTo(
        first.strengthScore,
      ),
    );
  ...
  List<AtlasIntelligenceHypothesis>
      _buildHypotheses({
    required List<AtlasIntelligenceSignal> signals,
      ...
      switch (pattern.type) {
        case AtlasIntelligencePatternType.bottleneck:
          result.add(
            AtlasIntelligenceHypothesis(
              id:
                  'hypothesis_${pattern.id}',
              title:
                  'Capacidade de execução insuficiente',
              description:
          ...
          result.add(
            AtlasIntelligenceHypothesis(
              id:
                  'hypothesis_${pattern.id}',
              title:
                  'Riscos com causa comum',
              description:
                  'Os sinais podem estar sendo provocados pelo mesmo fator operacional.',
              cause:
                  'Uma restrição compartilhada entre os indicadores.',
              effect:
                  pattern.expectedConsequence,
              probabilityPercent:
                  pattern.confidencePercent,
              impactScore:
                  pattern.strengthScore,
          ...
          result.add(
            AtlasIntelligenceHypothesis(
              id:
                  'hypothesis_${pattern.id}',
              title:
                  'Premissas de planejamento desalinhadas',
              description:
                  pattern.description,
              cause:
          ...
          result.add(
            AtlasIntelligenceHypothesis(
              id:
                  'hypothesis_${pattern.id}',
              title:
                  'Potencial de ganho ainda não capturado',
              description:
                  pattern.description,
              cause:
    ...
    if (result.isEmpty && signals.isNotEmpty) {
      ...
      result.add(
        AtlasIntelligenceHypothesis(
          ...
          title:
              'Sinal principal exige validação',
          description:
              top.description,
          cause:
              'O principal fator ainda não foi confirmado.',
          effect:
  ...
  List<AtlasIntelligenceRecommendation>
      _buildRecommendations({
    required List<AtlasIntelligenceSignal> signals,
    required List<AtlasIntelligencePattern> patterns,
      ...
      candidates.add(
        _RecommendationCandidate(
          ...
          expectedFinancialImpact:
              _impactFromHypothesis(
            hypothesis: hypothesis,
            signals: signals,
            patterns: patterns,
          ),
          deadlineHours:
              hypothesis.impactScore >= 80
                  ? 8
                  : 24,
          reasoning:
    ...
    for (final signal in signals.take(8)) {
      candidates.add(
        _RecommendationCandidate(
          id:
              'recommendation_signal_${signal.id}',
          title: signal.title,
          description:
              signal.description,
          farmName: signal.farmName,
          score:
              signal.relevanceScore * 0.60 +
          ...
          deadlineHours:
              signal.severity ==
                  ...
                  : signal.severity ==
                      ...
                      : 48,
          reasoning:
              'O sinal possui alta relevância e pode afetar a operação.',
          actions: const [
            'Validar o sinal.',
            'Definir responsável.',
  ...
  double _intelligenceScore({
    ...
    final patternPenalty =
        patterns.fold<double>(
      0,
      (sum, item) =>
          sum + item.strengthScore * 0.05,
    );

    return (atlasOs.healthScore -
            severeSignals * 3 -
            patternPenalty)
        .clamp(0.0, 100.0)
        .toDouble();
  ...
  double _confidence({
    ...
    final values = <double>[
      ...signals.map(
        (item) => item.confidencePercent,
      ),
      ...patterns.map(
        (item) => item.confidencePercent,
      ),
      ...hypotheses.map(
        (item) => item.probabilityPercent,
      ),
    ];
  ...
  AtlasIntelligenceSeverity _severityFromOs(
  ...
  AtlasIntelligencePriority _priorityFromScore(
    double score,
  ) {
    if (score >= 85) {
  ...
  String _farmFromHypothesis({
    ...
    final pattern = patterns
        .cast<AtlasIntelligencePattern?>()
        .firstWhere(
          (item) =>
              item?.id ==
              hypothesis.id.replaceFirst(
                'hypothesis_',
                '',
              ),
          orElse: () => null,
    ...
    if (pattern != null) {
      for (final signalId
          in pattern.relatedSignalIds) {
        final signal = signals
            .cast<AtlasIntelligenceSignal?>()
            .firstWhere(
              (item) =>
                  item?.id == signalId,
              orElse: () => null,
