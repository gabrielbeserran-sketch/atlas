...
class AtlasExecutiveBrainService {
  ...
  AtlasExecutiveBrainData build({
    ...
    final crossImpacts = _buildCrossImpacts(executiveCore);

    final conflicts = _buildConflicts(executiveCore);
    ...
    final plans = _buildPlans(executiveCore);

    final memoryInsights = _buildMemoryInsights(executiveCore);
    ...
    AtlasExecutiveBrainEventService.instance.publishIfChangedDetached(result);

    return result;
  }

  List<AtlasExecutiveCrossImpact> _buildCrossImpacts(
    AtlasExecutiveCoreData core,
  ) {
    final result = <AtlasExecutiveCrossImpact>[];

    if (core.operationalIndex < 60 && core.financialIndex >= 65) {
      result.add(
        AtlasExecutiveCrossImpact(
          id: 'operational_to_financial',
          title: 'Execução pode limitar o resultado financeiro',
          description:
          ...
          direction: AtlasExecutiveImpactDirection.negative,
          impactScore: (100 - core.operationalIndex)
              .clamp(0.0, 100.0)
              .toDouble(),
          probabilityPercent: 86,
          financialImpact: core.opportunities.fold<double>(
            0,
            (sum, item) => sum + item.expectedReturn * 0.25,
          ),
          recommendation:
    ...
    if (core.predictiveIndex < 60 && core.strategicIndex >= 70) {
      result.add(
        AtlasExecutiveCrossImpact(
          id: 'predictive_to_strategy',
          title: 'Baixa previsibilidade pode comprometer a estratégia',
          description:
              'A estratégia está estruturada, porém a confiança preditiva permanece limitada.',
          sourceArea: 'Preditivo',
          affectedArea: 'Estratégico',
          direction: AtlasExecutiveImpactDirection.negative,
          impactScore: (100 - core.predictiveIndex)
              .clamp(0.0, 100.0)
              .toDouble(),
    ...
    if (core.financialIndex >= 70 && core.operationalIndex >= 70) {
      result.add(
        AtlasExecutiveCrossImpact(
          id: 'operations_finance_opportunity',
          title: 'Execução e finanças alinhadas',
          description:
              'A combinação de boa capacidade operacional e oportunidade financeira favorece expansão controlada.',
          sourceArea: 'Operacional',
          affectedArea: 'Financeiro',
          direction: AtlasExecutiveImpactDirection.positive,
          impactScore: ((core.financialIndex + core.operationalIndex) / 2)
              .clamp(0.0, 100.0)
              .toDouble(),
    ...
    if (core.healthIndex < 55) {
      result.add(
        AtlasExecutiveCrossImpact(
          ...
          title: 'Saúde geral afeta toda a operação',
          description:
          ...
          direction: AtlasExecutiveImpactDirection.mixed,
          impactScore: (100 - core.healthIndex).clamp(0.0, 100.0).toDouble(),
          probabilityPercent: 90,
          financialImpact: core.risks.fold<double>(
            0,
  ...
  List<AtlasExecutiveConflict> _buildConflicts(AtlasExecutiveCoreData core) {
    final result = <AtlasExecutiveConflict>[];

    final urgentPriorities = core.priorities.where((item) {
    ...
    if (urgentPriorities.length >= 5) {
      result.add(
        AtlasExecutiveConflict(
          id: 'too_many_urgent_priorities',
          title: 'Excesso de prioridades urgentes',
          description:
              '${urgentPriorities.length} prioridades possuem prazo inferior ou igual a 24 horas.',
          type: AtlasExecutiveConflictType.priority,
          severity: urgentPriorities.length >= 8
              ? AtlasExecutiveBrainSeverity.critical
              : AtlasExecutiveBrainSeverity.high,
          relatedEntityIds: urgentPriorities.map((item) => item.id).toList(),
          recommendation:
    ...
    final highInvestmentOpportunities = core.opportunities.where((item) {
    ...
    if (highInvestmentOpportunities.length >= 2) {
      result.add(
        AtlasExecutiveConflict(
          id: 'investment_resource_conflict',
          title: 'Disputa por recursos financeiros',
          description:
              'Existem múltiplas oportunidades que exigem investimento elevado ao mesmo tempo.',
          type: AtlasExecutiveConflictType.resource,
          severity: AtlasExecutiveBrainSeverity.high,
          relatedEntityIds: highInvestmentOpportunities
              .map((item) => item.id)
              .toList(),
          recommendation:
    ...
    if (core.operationalIndex < 55 && core.priorities.length >= 10) {
      result.add(
        AtlasExecutiveConflict(
          id: 'execution_capacity_conflict',
          title: 'Capacidade de execução abaixo da demanda',
          description:
              'O volume de prioridades supera a capacidade operacional indicada pelo sistema.',
          type: AtlasExecutiveConflictType.execution,
          severity: AtlasExecutiveBrainSeverity.critical,
          relatedEntityIds: core.priorities
              .take(10)
              .map((item) => item.id)
              .toList(),
          recommendation:
    ...
    if (core.strategicIndex >= 75 && core.healthIndex < 55) {
      result.add(
        AtlasExecutiveConflict(
          id: 'strategy_health_conflict',
          title: 'Estratégia incompatível com a saúde atual',
          description:
              'As metas estratégicas estão avançadas em relação à condição atual da operação.',
          type: AtlasExecutiveConflictType.strategy,
          severity: AtlasExecutiveBrainSeverity.high,
  ...
  AtlasExecutiveBrainDecision? _buildOfficialDecision({
    required AtlasExecutiveCoreData executiveCore,
    required List<AtlasExecutiveConflict> conflicts,
    required List<AtlasExecutiveCrossImpact> crossImpacts,
  }) {
    final baseDecision = executiveCore.bestDecisionOfWeek;

    if (baseDecision == null) {
      return null;
    }

    final criticalConflicts = conflicts.where((item) {
      return item.severity == AtlasExecutiveBrainSeverity.critical;
    }).length;

    final negativeImpacts = crossImpacts.where((item) {
      return item.direction == AtlasExecutiveImpactDirection.negative;
    }).length;

    final adjustedScore =
        (baseDecision.score - criticalConflicts * 8 - negativeImpacts * 3)
            .clamp(0.0, 100.0)
            .toDouble();

    final priority = _priorityFromScore(adjustedScore);
    ...
    return AtlasExecutiveBrainDecision(
      id: 'official_${baseDecision.id}',
      title: baseDecision.title,
      description: baseDecision.description,
      farmName: baseDecision.farmName,
      priority: priority,
      score: adjustedScore,
      confidencePercent: baseDecision.confidencePercent,
      expectedFinancialImpact: baseDecision.expectedFinancialImpact,
      deadlineHours: baseDecision.deadlineHours,
      reasoning: '${baseDecision.reasoning}$conflictReason',
      actions: baseDecision.actions,
      expectedResult: baseDecision.expectedFinancialImpact > 0
  ...
  AtlasExecutiveBrainStrategy? _buildStrategy({
    required AtlasExecutiveCoreData executiveCore,
    required AtlasExecutiveBrainDecision? officialDecision,
    required List<AtlasExecutiveCrossImpact> crossImpacts,
    required List<AtlasExecutiveConflict> conflicts,
  }) {
    ...
    final financialWeight = executiveCore.financialIndex < 60 ? 30.0 : 20.0;

    final operationalWeight = executiveCore.operationalIndex < 60 ? 35.0 : 25.0;

    final strategicWeight = executiveCore.strategicIndex < 60 ? 25.0 : 20.0;

    final predictiveWeight =
        100 - financialWeight - operationalWeight - strategicWeight;
    ...
    return AtlasExecutiveBrainStrategy(
      id: 'strategy_${officialDecision.id}',
      ...
      successProbabilityPercent:
          (officialDecision.confidencePercent - conflicts.length * 3)
              .clamp(35.0, 95.0)
              .toDouble(),
      expectedFinancialImpact: totalImpact,
      pillars: [
        AtlasExecutiveStrategyPillar(
          position: 1,
          title: 'Financeiro',
          description: 'Proteger caixa, priorizar ROI e reduzir exposição.',
          weightPercent: financialWeight,
          target: 'Elevar o índice financeiro acima de 70.',
        ),
        AtlasExecutiveStrategyPillar(
          position: 2,
          title: 'Operacional',
          description: 'Remover gargalos e aumentar a capacidade de execução.',
          weightPercent: operationalWeight,
          target: 'Elevar execução e reduzir atrasos.',
        ),
        AtlasExecutiveStrategyPillar(
          position: 3,
          title: 'Estratégico',
          description: 'Alinhar metas, prioridades e recursos.',
          weightPercent: strategicWeight,
          target: 'Manter metas compatíveis com a realidade.',
        ),
        AtlasExecutiveStrategyPillar(
          position: 4,
          title: 'Preditivo',
          description: 'Atualizar dados e antecipar riscos.',
          weightPercent: predictiveWeight,
          target: 'Elevar confiança preditiva acima de 75%.',
        ),
      ],
    );
  }

  _ExecutivePlans _buildPlans(AtlasExecutiveCoreData core) {
    ...
    for (final item in core.priorities) {
      ...
      final action = AtlasExecutiveBrainAction(
        ...
        horizon: horizon,
        priority: _priorityFromCore(item.priority),
    ...
    List<AtlasExecutiveBrainAction> rank(
      ...
      final selected = items.take(limit).toList();
  ...
  List<AtlasExecutiveMemoryInsight> _buildMemoryInsights(
    AtlasExecutiveCoreData core,
  ) {
    final result = <AtlasExecutiveMemoryInsight>[];

    for (final item in core.memoryRecords.take(20)) {
      final type = switch (item.type) {
        AtlasExecutiveMemoryType.pattern =>
          AtlasExecutiveMemoryInsightType.recurringPattern,
        AtlasExecutiveMemoryType.risk =>
          AtlasExecutiveMemoryInsightType.historicalRisk,
        AtlasExecutiveMemoryType.opportunity =>
          AtlasExecutiveMemoryInsightType.repeatedOpportunity,
        AtlasExecutiveMemoryType.decision =>
          AtlasExecutiveMemoryInsightType.decisionLesson,
        AtlasExecutiveMemoryType.mission =>
          AtlasExecutiveMemoryInsightType.missionLesson,
      };
      ...
      result.add(
        AtlasExecutiveMemoryInsight(
          ...
          recommendation: _memoryRecommendation(type),
        ),
      );
    }
  ...
  double _brainScore({
    ...
    final negativeImpactPenalty = crossImpacts
        .where((item) {
          return item.direction == AtlasExecutiveImpactDirection.negative;
        })
        .fold<double>(0, (sum, item) => sum + item.impactScore * 0.04);

    final conflictPenalty = conflicts.fold<double>(
      0,
      (sum, item) => sum + _severityWeight(item.severity) * 2.5,
    );
  ...
  double _confidence({
    ...
    final values = <double>[
      ...
      if (officialDecision != null) officialDecision.confidencePercent,
      ...crossImpacts.map((item) => item.probabilityPercent),
    ];
  ...
  AtlasExecutiveBrainStatus _status(double score) {
    if (score >= 80) {
  ...
  AtlasExecutiveBrainPriority _priorityFromScore(double score) {
    if (score >= 85) {
  ...
  AtlasExecutiveBrainPriority _priorityFromCore(
  ...
  int _severityWeight(AtlasExecutiveBrainSeverity severity) {
    switch (severity) {
  ...
  String _memoryRecommendation(AtlasExecutiveMemoryInsightType type) {
    switch (type) {
  ...
  String _summary({
    ...
    final decisionTitle = officialDecision?.title ?? 'nenhuma decisão oficial';

    final strategyTitle = strategy?.title ?? 'nenhuma estratégia central';
  ...
  List<AtlasExecutiveScoreDimension> _buildScoreDimensions(
    AtlasExecutiveCoreData core,
  ) {
    return [
      AtlasExecutiveScoreDimension(
        title: 'Financeiro',
        score: core.financialIndex,
        weightPercent: 25,
        explanation: 'Liquidez, retorno, margem e capacidade de investimento.',
      ),
      AtlasExecutiveScoreDimension(
        title: 'Operacional',
        score: core.operationalIndex,
        weightPercent: 20,
        explanation: 'Execução, recursos, prazos e eficiência operacional.',
      ),
      AtlasExecutiveScoreDimension(
        title: 'Estratégico',
        score: core.strategicIndex,
        weightPercent: 20,
        explanation: 'Metas, prioridades, alinhamento e geração de valor.',
      ),
      AtlasExecutiveScoreDimension(
        title: 'Preditivo',
        score: core.predictiveIndex,
        weightPercent: 15,
        explanation: 'Qualidade das previsões, cenários e confiança dos dados.',
      ),
      AtlasExecutiveScoreDimension(
        title: 'Saúde geral',
        score: core.healthIndex,
        weightPercent: 20,
        explanation:
            'Visão consolidada da saúde técnica e gerencial da fazenda.',
      ),
    ];
  }
  ...
  List<AtlasExecutiveRadarItem> _buildRadarItems(AtlasExecutiveCoreData core) {
    ...
    for (final opportunity in core.opportunities.take(4)) {
      final quickWin =
          opportunity.investmentValue <= 20000 && opportunity.roiPercent >= 20;
      items.add(
        AtlasExecutiveRadarItem(
          ...
          type: quickWin
              ? AtlasExecutiveRadarType.quickWin
              : AtlasExecutiveRadarType.opportunity,
          priority: opportunity.roiPercent >= 30
              ? AtlasExecutiveBrainPriority.high
              : AtlasExecutiveBrainPriority.medium,
    ...
    items.sort((a, b) {
      ...
      return byPriority != 0
          ? byPriority
          : b.expectedFinancialImpact.abs().compareTo(
              a.expectedFinancialImpact.abs(),
            );
    });
