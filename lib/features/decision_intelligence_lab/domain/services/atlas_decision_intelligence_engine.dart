...
class AtlasDecisionIntelligenceEngine {
  ...
  AtlasDecisionScenarioResult simulate({
    ...
    final evidenceConfidence = recommendation?.confidence ?? 55;
    final historicSuccess = recommendation?.successRate ?? 68;

    final readinessFactor = scenario.executionReadiness.clamp(0.0, 100.0);
    final complexityPenalty = scenario.operationalComplexity.clamp(0.0, 100.0);
    ...
    final projectedGain =
        (successProbability / 100 * (recommendation?.expectedScoreGain ?? 12))
            .clamp(0.0, 30.0)
            .toDouble();

    final projectedAreaScore = math
        .min(100, currentScore + projectedGain)
        .toDouble();
    ...
    final expectedResultMonths = math.max(
      ...
      math.min(
        ...
        ((recommendation?.averageResponseDays ?? 90) / 30).ceil(),
      ),
    );

    final roiScore = ((roi + 20) / 2).clamp(0.0, 100.0).toDouble();
    final paybackScore = payback >= 999
        ? 0.0
        : (100 - payback * 5).clamp(0.0, 100.0).toDouble();
    ...
    return AtlasDecisionScenarioResult(
      id: 'decision_${scenario.id}_${DateTime.now().microsecondsSinceEpoch}',
      farmId: audit.farmId,
      farmName: audit.farmName,
      generatedAt: DateTime.now(),
      input: scenario,
      ...
      implementationPlan: _implementationPlan(scenario.area),
  ...
  List<String> _implementationPlan(AtlasFarmAuditArea area) {
    return <String>[
