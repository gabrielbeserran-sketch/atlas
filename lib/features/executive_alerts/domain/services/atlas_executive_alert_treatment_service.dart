...
class AtlasExecutiveAlertTreatmentService {
  ...
  List<AtlasExecutiveAlertTreatmentItem>
      merge({
    ...
    return alerts.map((alert) {
      ...
      final state = existing ??
          AtlasExecutiveAlertState(
            ...
            customDeadline: currentTime.add(
              Duration(
                days: alert.responseDeadlineDays,
              ),
            ),
          );

      return AtlasExecutiveAlertTreatmentItem(
        alert: alert,
        state: state,
      );
    }).toList()
      ..sort((first, second) {
        ...
        return second.alert.priorityScore.compareTo(
  ...
  AtlasExecutiveAlertState updateStatus({
    ...
    required AtlasExecutiveAlertTreatmentStatus
        status,
    DateTime? now,
  }) {
    final currentTime = now ?? DateTime.now();
    ...
    return state.copyWith(
      ...
      resolvedAt:
          status ==
                  AtlasExecutiveAlertTreatmentStatus
                      .resolved
              ? currentTime
              : null,
      clearResolvedAt:
  ...
  AtlasExecutiveAlertTreatmentProgress
      calculateProgress(
    ...
    final averageResolutionHours =
        durations.isEmpty
            ? 0.0
            : durations.fold<double>(
                  0,
                  (sum, duration) =>
                      sum + duration.inMinutes / 60,
                ) /
                durations.length;
  ...
  String buildProgressSummary({
    ...
    required List<AtlasExecutiveAlertTreatmentItem>
        items,
  }) {
    final progress =
