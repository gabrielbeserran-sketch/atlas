...
class AtlasBackendFoundationScreen extends StatefulWidget {
  ...
  @override
  State<AtlasBackendFoundationScreen> createState() =>
      _AtlasBackendFoundationScreenState();
}

class _AtlasBackendFoundationScreenState
    extends State<AtlasBackendFoundationScreen> {
  ...
  Future<void> load() async {
    ...
    loaded.sort(
      (a, b) => parseAtlasBackendDate(
        b.date,
      ).compareTo(parseAtlasBackendDate(a.date)),
    );
    if (!mounted) return;
    setState(() {
      records = loaded;
  ...
  List<AtlasBackendFoundationRecord> get visibleRecords => records
      .where(
        (e) =>
            e.module == selectedModule &&
            (selectedFeature == 'Todos' || e.feature == selectedFeature),
      )
      .toList();
                ...
                Wrap(
                  ...
                  children: [
                    EnterpriseMetricCard(
                      title: 'Cobertura',
                      value: '${analytics.coveragePercent.toStringAsFixed(0)}%',
                    ...
                    EnterpriseMetricCard(
                      title: 'Taxa de erro',
                      value:
                          '${analytics.averageErrorRate.toStringAsFixed(1)}%',
                      subtitle: 'Média',
                    ...
                    EnterpriseMetricCard(
                      title: 'Latência',
                      value:
                          '${analytics.averageLatency.toStringAsFixed(0)} ms',
                      subtitle: 'Média',
...
class _BackendFormState extends State<_BackendForm> {
  ...
  void initState() {
    ...
    c = {
      ...
      'date': TextEditingController(
        text: r?.date ?? formatAtlasBackendDate(DateTime.now()),
      ),
      ...
      'progress': TextEditingController(
        text: r == null ? '' : r.progressPercent.toString(),
      ),
      'availability': TextEditingController(
        text: r == null ? '' : r.availabilityPercent.toString(),
      ),
      'error': TextEditingController(
        text: r == null ? '' : r.errorRatePercent.toString(),
      ),
      'latency': TextEditingController(
        text: r == null ? '' : r.latencyMs.toString(),
      ),
      'alerts': TextEditingController(
        text: r == null ? '' : r.alertCount.toString(),
      ),
  ...
  void save() {
    ...
    Navigator.pop(
      context,
      AtlasBackendFoundationRecord(
        id:
            widget.current?.id ??
            'backend_${DateTime.now().microsecondsSinceEpoch}',
...
IconData _moduleIcon(AtlasBackendFoundationModule module) => switch (module) {
  ...
  AtlasBackendFoundationModule.environmentConfiguration =>
  ...
  AtlasBackendFoundationModule.multiCompanyArchitecture =>
  ...
  AtlasBackendFoundationModule.animalsApi => AtlasLivestockIcons.cow,
  AtlasBackendFoundationModule.livestockEventsApi => Icons.event_note_outlined,
  AtlasBackendFoundationModule.backendAdministrationCenter =>
