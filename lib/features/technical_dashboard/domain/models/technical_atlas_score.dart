...
class TechnicalAtlasScore {
  ...
  static double _herdScore(TechnicalFarmSummary s) {
    ...
    if (s.totalAnimals > 0 && s.activeAnimals / s.totalAnimals < 0.5) {
      score -= 10;
    }
  ...
  static double _reproductionScore(TechnicalFarmSummary s) {
    var score = 100.0;
    if (s.reproductionRecords == 0) {
      score -= 35;
    }
    score -= s.overdueReproductionEvents * 8;
    if (s.reproductionRecords > 0 && s.positivePregnancies == 0) {
      score -= 15;
    }
    return score.clamp(0, 100).toDouble();
  }
  ...
  static double _nutritionScore(TechnicalFarmSummary s) {
    ...
    var score = 100.0;
    if (s.nutritionPlans == 0) {
      return 35;
    }
    final coverage = s.nutritionAnimals / s.activeAnimals;
    if (coverage < 1) {
      score -= (1 - coverage.clamp(0, 1)) * 45;
    }
    if (s.dailyFeedKg <= 0) {
      score -= 15;
    }
