...
class _AtlasInvestmentCapitalScreenState
  ...
  Future<void> _load() async {
    ...
    final filtered = widget.farmId == null
        ? all
        : all
            .where((item) => item.farmId.isEmpty || item.farmId == widget.farmId)
            .toList();
    if (!mounted) return;
    setState(() {
      _projects = filtered;
      _portfolio = _engine.optimize(projects: filtered, constraint: _constraint);
      _loading = false;
    });
  }
          ...
          IconButton(
            ...
            onPressed: () => Navigator.of(context).push(
              MaterialPageRoute<void>(
                builder: (_) => AtlasExecutionEngineScreen(farmId: widget.farmId),
              ),
            ),
                        ...
                        if (portfolio.selected.isEmpty)
                          const Card(
                            child: Padding(
                              ...
                              child: Text('Nenhum projeto selecionado com as restrições atuais.'),
                            ),
                          )
                        else
          ...
          children: [
            const SizedBox(
              ...
              child: Column(
                ...
                children: [
                  Text('Plano ótimo de alocação de capital',
...
class _MetricsGrid extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final items = <(String, String, IconData)>[
      ('Capital alocado', currency.format(portfolio.allocatedCapital), Icons.savings_outlined),
      ('Saldo disponível', currency.format(portfolio.remainingCapital), Icons.account_balance_wallet_outlined),
      ('VPL do portfólio', currency.format(portfolio.portfolioNpv), Icons.trending_up),
      ('ROI global', '${portfolio.portfolioRoi.toStringAsFixed(1)}%', Icons.percent),
      ('Risco médio', '${portfolio.averageRisk.toStringAsFixed(0)}/100', Icons.shield_outlined),
      ('Payback médio', '${portfolio.averagePayback.toStringAsFixed(1)} anos', Icons.schedule),
    ];
    return Wrap(
      spacing: 12,
      runSpacing: 12,
          ...
          child: Card(
            child: Padding(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Text(item.$2, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 17)),
              ]),
            ),
          ),
        );
  ...
  Widget build(BuildContext context) {
    ...
    return Card(
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('Estrutura de financiamento', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
          const SizedBox(height: 12),
          Wrap(spacing: 28, runSpacing: 10, children: [
            Text('Valor financiado: ${currency.format(financing.financedAmount)}'),
            Text('Parcela mensal: ${currency.format(financing.monthlyPayment)}'),
            Text('Juros totais: ${currency.format(financing.totalInterest)}'),
          ]),
          const SizedBox(height: 12),
    ...
    return Card(
      child: ExpansionTile(
        leading: CircleAvatar(
          backgroundColor: selected ? const Color(0xFFE0F2F1) : const Color(0xFFF1F3F4),
          child: Icon(selected ? Icons.check_circle_outline : Icons.analytics_outlined,
        ...
        subtitle: Text('${atlasInvestmentCategoryLabel(project.category)} • ${atlasInvestmentDecisionLabel(item.decision)}'),
        trailing: Text('${item.priorityScore.toStringAsFixed(0)}/100', style: const TextStyle(fontWeight: FontWeight.bold)),
        children: [
          Padding(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Wrap(spacing: 20, runSpacing: 10, children: [
                ...
                Text('Payback: ${item.paybackYears.toStringAsFixed(1)} anos'),
...
class _ConstraintDialogState extends State<_ConstraintDialog> {
  ...
  void initState() {
    super.initState();
    cash = TextEditingController(text: widget.initial.availableCash.toStringAsFixed(0));
    budget = TextEditingController(text: widget.initial.annualBudget.toStringAsFixed(0));
    debt = TextEditingController(text: widget.initial.maximumDebt.toStringAsFixed(0));
    rate = TextEditingController(text: widget.initial.interestRate.toStringAsFixed(1));
    years = TextEditingController(text: widget.initial.financingYears.toString());
    commitment = TextEditingController(text: widget.initial.maximumCashCommitment.toStringAsFixed(0));
    discount = TextEditingController(text: widget.initial.discountRate.toStringAsFixed(1));
  }
...
class _ProjectDialogState extends State<_ProjectDialog> {
      ...
      content: SizedBox(
        ...
        child: SingleChildScrollView(
          child: Column(children: [
            ...
            const SizedBox(height: 12),
            TextField(controller: description, maxLines: 2, decoration: const InputDecoration(labelText: 'Descrição', border: OutlineInputBorder())),
            const SizedBox(height: 12),
            DropdownButtonFormField<AtlasInvestmentCategory>(
              initialValue: category,
              decoration: const InputDecoration(labelText: 'Categoria', border: OutlineInputBorder()),
              items: AtlasInvestmentCategory.values.map((item) => DropdownMenuItem(value: item, child: Text(atlasInvestmentCategoryLabel(item)))).toList(),
            ...
            const SizedBox(height: 14),
            _slider('Alinhamento estratégico', alignment, (value) => setState(() => alignment = value)),
            _slider('Capacidade operacional', capacity, (value) => setState(() => capacity = value)),
  ...
  Widget _slider(String label, double value, ValueChanged<double> onChanged) => Column(
        ...
        children: [
          ...
          Slider(value: value, min: 0, max: 100, divisions: 20, onChanged: onChanged),
        ],
      );
}
