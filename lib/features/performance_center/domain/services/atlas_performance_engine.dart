...
class AtlasPerformanceEngine {
  ...
  AtlasPerformanceSnapshot generate({
    ...
    final completed = plan.missions
        .where((mission) => mission.status == AtlasMissionStatus.completed)
        .toList();
    ...
    final onTimeRate = completed.isEmpty
        ? 0.0
        : completedOnTime / completed.length * 100;
    final planProgress = plan.progressPercent;
    final auditEvolution = previousAudit == null
        ? 0.0
        : currentAudit.overallIndex - previousAudit.overallIndex;
    final overduePenalty = plan.totalMissions == 0
        ? 0.0
        : plan.overdueMissions / plan.totalMissions * 25;
    final impactRate = plan.expectedImpact <= 0
        ? 0.0
        : (plan.realizedImpact / plan.expectedImpact * 100)
              .clamp(0.0, 100.0)
              .toDouble();
    ...
    final kpis = currentAudit.areaResults.map((area) {
      final previous = previousAudit?.areaResults
          .where((item) => item.area == area.area)
          .firstOrNull;
      final before =
          previous?.score ??
          (area.score - _missionGain(plan, area.area))
              .clamp(0.0, 100.0)
              .toDouble();
    ...
    }).toList()..sort((a, b) => a.currentValue.compareTo(b.currentValue));
    ...
    if (plan.overdueMissions > 0) {
      alerts.add(
        AtlasPerformanceAlert(
          ...
          message:
              '${plan.overdueMissions} missão(ões) ultrapassaram o prazo e precisam de revisão imediata.',
          severity: plan.overdueMissions >= 3
              ? AtlasPerformanceAlertSeverity.critical
              : AtlasPerformanceAlertSeverity.high,
    ...
    for (final kpi
        in kpis
            .where(
              (item) =>
                  item.trend == AtlasPerformanceTrend.worsening ||
                  item.currentValue < 55,
            )
            .take(5)) {
      alerts.add(
        AtlasPerformanceAlert(
          ...
          message:
              '${kpi.title} está em ${kpi.currentValue.toStringAsFixed(1)} pontos e apresenta tendência ${atlasPerformanceTrendLabel(kpi.trend).toLowerCase()}.',
          severity: kpi.currentValue < 45
              ? AtlasPerformanceAlertSeverity.critical
              : AtlasPerformanceAlertSeverity.high,
          area: kpi.area,
        ),
    ...
    if (auditEvolution < -2) {
      alerts.add(
        AtlasPerformanceAlert(
          ...
          message:
              'O Atlas Farm Audit Index caiu ${auditEvolution.abs().toStringAsFixed(1)} pontos desde a auditoria anterior.',
    ...
    if (alerts.isEmpty) {
      alerts.add(
        const AtlasPerformanceAlert(
          ...
          message:
              'Nenhum alerta crítico foi identificado no ciclo atual de execução.',
