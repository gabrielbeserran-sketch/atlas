enum AtlasEnterpriseSyncOperationType {
  create,
  update,
  delete,
  restore,
...
String atlasEnterpriseSyncStatusLabel(
  AtlasEnterpriseSyncStatus value,
) {
  switch (value) {
...
class AtlasEnterpriseSyncOperation {
  ...
  factory AtlasEnterpriseSyncOperation.fromMap(
    ...
    return AtlasEnterpriseSyncOperation(
      ...
      operationType:
          AtlasEnterpriseSyncOperationType.values.firstWhere(
        (item) =>
            item.name == map['operationType']?.toString(),
        orElse: () =>
      ...
      baseVersion:
          (map['baseVersion'] as num?)?.toInt() ?? 0,
      createdAt:
          DateTime.tryParse(
          ...
          DateTime.now(),
      deviceId:
          map['deviceId']?.toString() ?? 'local_device',
...
class AtlasEnterpriseSyncConflict {
  ...
  factory AtlasEnterpriseSyncConflict.fromMap(
    Map<String, dynamic> map,
  ) {
    return AtlasEnterpriseSyncConflict(
      id: map['id']?.toString() ?? '',
      ...
      entityId: map['entityId']?.toString() ?? '',
      localVersion:
          (map['localVersion'] as num?)?.toInt() ?? 0,
      remoteVersion:
          (map['remoteVersion'] as num?)?.toInt() ?? 0,
      localPayload: Map<String, dynamic>.from(
        (map['localPayload'] as Map?) ??
            const <String, dynamic>{},
      ),
      remotePayload: Map<String, dynamic>.from(
        (map['remotePayload'] as Map?) ??
            const <String, dynamic>{},
      ),
      detectedAt:
          DateTime.tryParse(
            map['detectedAt']?.toString() ?? '',
          ) ??
          DateTime.now(),
      resolution:
          AtlasEnterpriseConflictResolution.values.firstWhere(
        (item) =>
            item.name == map['resolution']?.toString(),
        orElse: () =>
            AtlasEnterpriseConflictResolution.unresolved,
      ),
      resolvedAt: DateTime.tryParse(
        map['resolvedAt']?.toString() ?? '',
      ),
...
class AtlasEnterpriseSyncCheckpoint {
  ...
  factory AtlasEnterpriseSyncCheckpoint.fromMap(
    Map<String, dynamic> map,
  ) {
    return AtlasEnterpriseSyncCheckpoint(
      companyId: map['companyId']?.toString() ?? '',
      ...
      lastPulledAt: DateTime.tryParse(
        map['lastPulledAt']?.toString() ?? '',
      ),
      lastPushedAt: DateTime.tryParse(
        map['lastPushedAt']?.toString() ?? '',
      ),
    );
  }
}
