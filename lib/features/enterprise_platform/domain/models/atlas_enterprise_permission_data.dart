...
enum AtlasPermissionEffect { allow, deny }
...
abstract final class AtlasEnterprisePermissions {
  ...
  static Set<String> keysWhere(
    ...
    return catalog.where(test).map((permission) => permission.key).toSet();
  }

  static Set<String> defaultsForRole(AtlasEnterpriseMembershipRole role) {
    final all = catalog.map((item) => item.key).toSet();
    final readOnly = keysWhere((item) => item.operation == 'read');
...
class AtlasCustomEnterpriseRole {
  ...
  factory AtlasCustomEnterpriseRole.fromMap(Map<String, dynamic> map) {
    return AtlasCustomEnterpriseRole(
      id: map['id']?.toString() ?? '',
      companyId: map['companyId']?.toString() ?? '',
      name: map['name']?.toString() ?? '',
      description: map['description']?.toString() ?? '',
      permissionKeys: ((map['permissionKeys'] as List?) ?? const <dynamic>[])
...
class AtlasUserPermissionPolicy {
  ...
  Map<String, dynamic> toMap() => <String, dynamic>{
    ...
    'effects': effects.map((key, value) => MapEntry(key, value.name)),
    'updatedBy': updatedBy,
    'updatedAt': updatedAt.toIso8601String(),
  };

  factory AtlasUserPermissionPolicy.fromMap(Map<String, dynamic> map) {
...
class AtlasEffectivePermissionSet {
  ...
  bool allows(String permissionKey) =>
      allowed.contains(permissionKey) && !denied.contains(permissionKey);
}
