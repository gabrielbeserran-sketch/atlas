...
class AtlasOptimizationScreen
    extends StatefulWidget {
  const AtlasOptimizationScreen({
    super.key,
  });

  @override
  State<AtlasOptimizationScreen>
      createState() {
    return _AtlasOptimizationScreenState();
  }
}

class _AtlasOptimizationScreenState
    extends State<AtlasOptimizationScreen> {
  final formKey = GlobalKey<FormState>();

  final maxInvestmentController =
      TextEditingController(text: '100000');

  final maxHerdExpansionController =
  ...
  final AtlasOptimizationEngine engine =
  ...
  Future<void> _optimize() async {
    ...
    try {
      ...
      final request = AtlasOptimizationRequest(
        id:
            'optimization_${DateTime.now().microsecondsSinceEpoch}',
        farmId: twin.farmId,
        farmName: twin.farmName,
        objective: objective,
        horizonMonths: horizonMonths,
        maxInvestment: _parseDouble(
          maxInvestmentController.text,
        ),
        maxRisk: riskTolerance,
        maxHerdExpansion: _parseInt(
          maxHerdExpansionController.text,
        ),
...
class _ObjectiveCard extends StatelessWidget {
  ...
  final ValueChanged<AtlasOptimizationObjective>
      onObjectiveChanged;
        ...
        child: Column(
          children: [
            ...
            DropdownButtonFormField<
              ...
              items:
                  AtlasOptimizationObjective.values
                      .map((item) {
                return DropdownMenuItem<
                    AtlasOptimizationObjective>(
                  value: item,
                  child: Text(
                    atlasOptimizationObjectiveLabel(
                      item,
                    ),
                  ),
                );
...
class _ConstraintCard extends StatelessWidget {
  ...
  final TextEditingController
      maxHerdExpansionController;
  final AtlasOptimizationRiskTolerance
      riskTolerance;
  final ValueChanged<
      AtlasOptimizationRiskTolerance> onRiskChanged;
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            ...
            const SizedBox(height: 14),
            DropdownButtonFormField<
                AtlasOptimizationRiskTolerance>(
              initialValue: riskTolerance,
              decoration: const InputDecoration(
                labelText:
                    'Tolerância máxima ao risco',
                border: OutlineInputBorder(),
                prefixIcon:
                    Icon(Icons.shield_outlined),
              ),
              items:
                  AtlasOptimizationRiskTolerance.values
                      .map((item) {
                return DropdownMenuItem<
                    AtlasOptimizationRiskTolerance>(
                  value: item,
                  child: Text(
                    atlasOptimizationRiskToleranceLabel(
                      item,
                    ),
                  ),
                );
