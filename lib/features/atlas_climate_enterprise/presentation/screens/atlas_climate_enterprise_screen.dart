...
class AtlasClimateEnterpriseScreen extends StatefulWidget {
  const AtlasClimateEnterpriseScreen({
  ...
  final AtlasClimateEnterpriseModule initialModule;

  @override
  State<AtlasClimateEnterpriseScreen> createState() =>
      _AtlasClimateEnterpriseScreenState();
}
...
class _ClimateForm extends StatefulWidget {
  ...
  @override
  State<_ClimateForm> createState() => _ClimateFormState();
}

class _ClimateFormState extends State<_ClimateForm> {
  ...
  double percent(TextEditingController controller) =>
      decimal(controller).clamp(0, 100);

  int nonNegative(TextEditingController controller) =>
      integer(controller) < 0 ? 0 : integer(controller);
  ...
  Future<void> chooseDate() async {
    ...
    setState(() {
      date.text = formatAtlasClimateDate(selected);
    });
  }
