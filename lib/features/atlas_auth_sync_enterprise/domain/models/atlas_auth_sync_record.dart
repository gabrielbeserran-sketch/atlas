...
extension AtlasAuthSyncModuleX on AtlasAuthSyncModule {
  String get code => switch (this) {
    AtlasAuthSyncModule.secureUserRegistration => 'secure_user_registration',
    AtlasAuthSyncModule.secureTokenLogin => 'secure_token_login',
    AtlasAuthSyncModule.passwordRecovery => 'password_recovery',
    AtlasAuthSyncModule.multiFactorAuthentication =>
      'multi_factor_authentication',
    AtlasAuthSyncModule.roleBasedAccessControl => 'role_based_access_control',
    AtlasAuthSyncModule.sensitiveDataProtection => 'sensitive_data_protection',
    AtlasAuthSyncModule.immutableAuditLogs => 'immutable_audit_logs',
    AtlasAuthSyncModule.structuredOfflineDatabase =>
      'structured_offline_database',
    AtlasAuthSyncModule.synchronizationEngine => 'synchronization_engine',
    AtlasAuthSyncModule.realConflictResolution => 'real_conflict_resolution',
  };

  String get title => switch (this) {
    AtlasAuthSyncModule.secureUserRegistration => 'Cadastro Seguro de Usuário',
    AtlasAuthSyncModule.secureTokenLogin => 'Login com Tokens Seguros',
    AtlasAuthSyncModule.passwordRecovery => 'Recuperação de Senha',
    AtlasAuthSyncModule.multiFactorAuthentication => 'Autenticação Multifator',
    AtlasAuthSyncModule.roleBasedAccessControl =>
      'Controle de Acesso por Papéis',
    AtlasAuthSyncModule.sensitiveDataProtection =>
      'Proteção de Dados Sensíveis',
    AtlasAuthSyncModule.immutableAuditLogs => 'Logs Imutáveis de Auditoria',
    AtlasAuthSyncModule.structuredOfflineDatabase => 'Banco Local Offline',
    AtlasAuthSyncModule.synchronizationEngine => 'Motor de Sincronização',
...
class AtlasAuthSyncRecord {
  ...
  factory AtlasAuthSyncRecord.fromMap(Map<String, dynamic> map) {
    final code = map['module']?.toString() ?? '';
    final module = AtlasAuthSyncModule.values.firstWhere(
      (item) => item.code == code,
      orElse: () => AtlasAuthSyncModule.secureUserRegistration,
    );
