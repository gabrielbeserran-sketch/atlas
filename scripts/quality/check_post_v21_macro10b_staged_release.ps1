...
Write-Host "=== ATLAS POS-V21 MACROPACOTE 10B - CHECK DO STAGING ===" -ForegroundColor Cyan
...
if ($unexpected.Count -gt 0) {
  ...
  throw "Staging contaminado por arquivos fora do 10B."
}
if ($missing.Count -gt 0) {
  $missing | ForEach-Object { Write-Host "  $_" -ForegroundColor Red }
  throw "Staging incompleto para o 10B."
}
...
if ($unchanged.Count -gt 0) { Write-Host "[OK] $($unchanged.Count) arquivo(s) do manifesto já idêntico(s) ao HEAD." -ForegroundColor Green }
Write-Host "ATLAS 10B STAGED RELEASE: APROVADO" -ForegroundColor Green
