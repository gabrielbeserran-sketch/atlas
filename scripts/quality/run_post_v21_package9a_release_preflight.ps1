...
Write-Host "=== ATLAS POS-V21 PACOTE 9A - PREFLIGHT DE RELEASE ===" -ForegroundColor Cyan

& $Python.Source tools\atlas_post_v21_package9a_release_gate.py
if ($LASTEXITCODE -ne 0) { throw "Gate de release 9A falhou." }

if (-not (Test-Path $MigrationPath)) {
    throw "Migration 0044 ausente."
}

$MigrationContent = Get-Content -Raw -Path $MigrationPath
if ($MigrationContent -notmatch 'revision\s*=\s*"20260823_0044"') {
    throw "Revision da migration 0044 incorreta."
}
if ($MigrationContent -notmatch 'down_revision\s*=\s*"20260823_0043"') {
    throw "Migration 0044 não encadeia 0043."
}

git check-ignore -q -- $MigrationPath
if ($LASTEXITCODE -eq 0) {
    throw "Migration 0044 está ignorada pelo Git."
}
...
Write-Host ""
Write-Host "ATLAS 9A RELEASE PREFLIGHT: APROVADO" -ForegroundColor Green
Write-Host "Próxima ordem:" -ForegroundColor Green
Write-Host "  git add -A" -ForegroundColor White
Write-Host "  powershell -ExecutionPolicy Bypass -File `".\scripts\quality\check_post_v21_package9a_staged_release.ps1`"" -ForegroundColor White
